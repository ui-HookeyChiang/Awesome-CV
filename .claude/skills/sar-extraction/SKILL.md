...
**Use the template**: copy `src/present/fragments/case-studies/_template.html` and replace all `{{PLACEHOLDER}}` values with extracted content — see `references/cheat-sheet-templates.md` for the field-by-field fill guide (frontmatter schema, Situation/Action/Result sections, cheat-sheet entries, ID conventions).
