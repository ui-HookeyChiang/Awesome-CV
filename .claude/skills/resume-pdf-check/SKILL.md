...
```bash
pdfinfo src/resume.pdf | rg Pages
```
...
### 3. Fixes for Overflow, by File

| Overflow Source | File to Trim | Fix |
|----------------|--------------|-----|
...
| Page 2 (education + scholar + leadership) | `src/resume/scholar.tex` | Auto-shrink in order: (1) remove least relevant project, (2) shorten remaining descriptions to single-line, (3) remove another project (keep top 1), (4) collapse to minimal format — remove `\begin{cvitems}`, use empty description braces `{}`. Rebuild and re-check page count after each step. For job builds, save the shrunk version to `resumes/<target>/resume/scholar.tex`. |
| Page 2 (education + scholar + leadership) | `src/resume/leadership.tex` | Consolidate entries |

### 4. Check for LaTeX Warnings

```bash
rg 'Overfull|Underfull' src/resume.log
```
...
```bash
cd src && xelatex resume.tex && echo "---" && pdfinfo resume.pdf | rg Pages && rg 'Overfull|Underfull' resume.log || echo "No box warnings"
```
