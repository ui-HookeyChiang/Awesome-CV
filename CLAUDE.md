...
## File Structure
- `src/resume/*.tex` - **Canonical GP resume** (all 5 .tex files — source of truth, edit here)
- `src/resume.tex` - Main resume document (imports from `resume/` relative path)
- `resumes/<company>/` - Job-targeted resumes with analysis reports (gitignored)
...
## Key Files
- `src/resume/*.tex` - **Canonical GP resume files** (edit here)
...
Uses the five canonical role strings unchanged (needs-triage/needs-info/ready-for-agent/ready-for-human/wontfix). See `docs/agents/triage-labels.md`.
